...
\subsection{Encapsulation using Shadow DOM}\label{sec:shadow_dom}
...
This session token is stored within the extension's \texttt{localStorage} IndexedDB location, instead of a per-origin \texttt{localStorage}. Although unencrypted, we can ensure that untrusted JavaScript on third-party web pages do not have access to the session token to open up the system to CSRF.
